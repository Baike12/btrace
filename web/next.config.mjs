...
const nextConfig = {
  ...
  // tRPC (/api/trpc/*) 和 NextAuth (/api/auth/[...nextauth]) 保留在 Next.js 本地处理
  async rewrites() {
    const rustBackend = process.env.RUST_API_URL || "http://localhost:8010";
    const routes = [
      "projects", "organizations", "traces", "observations",
      "scores", "sessions", "datasets", "prompts", "models",
      "dashboards", "comments", "media", "evals", "users",
      "automations", "monitors", "api-keys",
    ];
    const rewrites = routes.flatMap((r) => [
      { source: `/api/${r}/:path*`, destination: `${rustBackend}/api/${r}/:path*` },
      { source: `/api/${r}`, destination: `${rustBackend}/api/${r}` },
    ]);
    // Singular project route proxy
    rewrites.push({ source: "/api/project/:path*", destination: `${rustBackend}/api/projects/:path*` });
    rewrites.push({ source: "/api/project", destination: `${rustBackend}/api/projects` });
    // Auth routes proxy to Rust
    rewrites.push({ source: "/api/auth/login", destination: `${rustBackend}/api/auth/login` });
    rewrites.push({ source: "/api/auth/signup", destination: `${rustBackend}/api/auth/signup` });
    rewrites.push({ source: "/api/auth/session", destination: `${rustBackend}/api/auth/session` });
    rewrites.push({ source: "/api/auth/logout", destination: `${rustBackend}/api/auth/logout` });
    // Public ingestion proxy to Rust
    rewrites.push({ source: "/api/public/:path*", destination: `${rustBackend}/api/public/:path*` });
    // OpenAPI
    rewrites.push({ source: "/api/openapi.json", destination: `${rustBackend}/api/openapi.json` });
    return rewrites;
  },
