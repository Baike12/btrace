...
dd.init({
  // PG-only memory fix: runtimeMetrics 每 10s 采样 V8 heap，触发周期性 GC
  // 开销叠加在 Next.js dev 模块图上。本地开发关闭；生产如有 Datadog agent 再开。
...
const sdk = new NodeSDK({
  ...
  instrumentations: [
    // PG-only memory fix: IORedisInstrumentation + BullMQInstrumentation + AwsInstrumentation
    // 在 PG-only fork 已无对应后端，不再 require 这三个包，避免为它们注册 hook 与 metadata buffer。
