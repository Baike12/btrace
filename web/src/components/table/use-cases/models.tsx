...
const modelConfigDescriptions = {
  ...
  maintainer:
    "Maintainer of the model. Built-in models can be cloned, user managed models can be edited and deleted. To supersede a built-in model, set the custom model name to the built-in model name.",
...
export default function ModelTable({ projectId }: { projectId: string }) {
  ...
  const columns: LangfuseColumnDef<ModelTableRow>[] = [
    ...
    },
    {
      ...
      size: 60,
      cell: ({ row }) => {
        const isLangfuse = row.original.maintainer === "Built-in";
        return (
          <div className="flex justify-center">
            <Tooltip>
              ...
              <TooltipContent>
                {isLangfuse ? "Built-in" : "User maintained"}
              </TooltipContent>
            </Tooltip>
          </div>
        );
    ...
    },
    {
      ...
      size: 120,
      cell: ({ row }) => {
        return row.original.maintainer !== "Built-in" ? (
          <div
  ...
  const convertToTableRow = (model: GetModelResult): ModelTableRow => {
    ...
    return {
      modelId: model.id,
      maintainer: model.projectId ? "User" : "Built-in",
