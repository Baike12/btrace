...
import { type ObservationReturnType } from "@/src/types/server-types";
...
/**
 * Helper: get a Date from an observation field, handling both
 * camelCase (e.g. startTime, from autoParseDates conversion) and
 * snake_case (e.g. start_time, raw Rust API response).
 */
function getObsDate(obs: any, camelKey: string, snakeKey: string): Date | null {
  const val = obs[camelKey] ?? obs[snakeKey];
...
function prepareObservations(list: ObservationReturnType[]): {
  ...
  // Sort by start time (handle both camelCase and snake_case from API)
  const sortedObservations = mutableList.sort((a, b) => {
    const aTime = getObsDate(a, "startTime", "start_time")?.getTime() ?? 0;
    const bTime = getObsDate(b, "startTime", "start_time")?.getTime() ?? 0;
    return aTime - bTime;
  });
...
function buildTreeNodesBottomUp(
  ...
  while (queueIndex < queue.length) {
    ...
    const obsStartTime = getObsDate(obs, "startTime", "start_time") ?? new Date(0);
    const startTimeSinceTrace = obsStartTime.getTime() - traceStartTime.getTime();
    ...
    if (obs.parentObservationId) {
      ...
      if (parentNode) {
        const parentStart = getObsDate(parentNode.observation, "startTime", "start_time") ?? new Date(0);
        startTimeSinceParentStart = obsStartTime.getTime() - parentStart.getTime();
      }
    }
    ...
    const treeNode: TreeNode = {
      id: obs.id,
      type: obs.type ?? (obs as any).type,
      name: obs.name ?? "",
      startTime: getObsDate(obs, "startTime", "start_time") ?? new Date(0),
      endTime: getObsDate(obs, "endTime", "end_time"),
      level: obs.level,
...
function buildTraceTree(
  ...
  rootTreeNodes.sort((a, b) => (a.startTime?.getTime() ?? 0) - (b.startTime?.getTime() ?? 0));
