...
// PG-only: local measureAndReturn stub (ClickHouse removed)
const measureAndReturn = <T>(opts: { operationName: string; projectId?: string; input?: unknown; fn: (input: unknown) => Promise<T> }): Promise<T> => opts.fn(opts.input);
...
import {
  orderByToPgSql,
  orderByToEntries,
} from "../queries/pg-sql/orderby-factory";
import type {
...
} from "../queries/pg-sql/query-fragments";
...
import {
  ...
  OrderByEntry,
} from "../queries/pg-sql/event-query-builder";
import {
  shouldUseObservationsSubqueryRewrite,
  shouldRunObservationsShadowQuery,
} from "../queries/pg-sql/query-options";
...
const BATCH_IO_STRING_RENDERING_PROPS: RenderingProps = {
  // Batch I/O truncation is handled in SQL via left() for performance.
  truncated: false,
 ...
 * Common pattern: find time filter and convert to ISO string format
 */
export function extractTimeFilter(
  ...
  return timeFilter
    ? (timeFilter as DateTimeFilter).value.toISOString()
    : null;
}
...
export const getEventsStreamForEval = async (props: {
  ...
  const asyncGenerator = queryPgStream<EvalEventRow>({
    query,
    params: queryParams,
    tags: {
      feature: "batch-eval",
...
async function getObservationByIdFromEventsTableInternal({
  ...
  return await queryPg<EventsObservationRecordReadType>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getTraceByIdFromEventsTable = async ({
  ...
  if (excludeInputOutput) {
    ...
    queryBuilder
      .select(
        `left(t.input, ${env.LANGFUSE_SERVER_SIDE_IO_CHAR_LIMIT}) as input`,
      )
      .select(
        `left(t.output, ${env.LANGFUSE_SERVER_SIDE_IO_CHAR_LIMIT}) as output`,
      );
  } else {
...
function applyCursorPagination(
  ...
  queryBuilder = queryBuilder.when(Boolean(opts.cursor), (b) => {
    ...
    return b.whereRaw(
      ...
      {
        lastStartTime: cursor.lastStartTimeTo.toISOString(),
...
async function getObservationsCountFromEventsTableForPublicApiInternal(
  ...
  const result = await measureAndReturn({
    ...
    fn: async (input) => {
      return await queryPg<{ count: string }>({
        query,
        params: input.params,
        tags: input.tags,
      });
...
/**
 * Update events in PostgreSQL based on selector and updates provided.
...
export const updateEvents = async (
  ...
  for (const key of updateableEventKeys) {
    if (updates[key] !== undefined) {
      ...
      setClauses.push(`${key} = $${updateParams.length}::boolean`);
    }
  }
  ...
  whereParts.push(`project_id = $${whereParams.length}`);

  if (selector.spanIds && selector.spanIds.length > 0) {
    whereParams.push(selector.spanIds);
    whereParts.push(`span_id = ANY($${whereParams.length}::text[])`);
  }
  if (selector.traceIds && selector.traceIds.length > 0) {
    whereParams.push(selector.traceIds);
    whereParts.push(`trace_id = ANY($${whereParams.length}::text[])`);
  }
  if (selector.rootOnly === true) {
    whereParts.push(`parent_span_id = ''`);
  }

  const whereClause = `WHERE ${whereParts.join(" AND ")}`;

  const updateQuery = (table: string) => ({
    query: `UPDATE ${table} SET ${setClauses.join(", ")} ${whereClause}`,
    params: [...updateParams, ...whereParams],
    tags: { type: table, kind: "update", projectId },
  });

  await Promise.all([
    executePg(updateQuery("events_full")),
    executePg(updateQuery("events_core")),
  ]);
};
...
export const getEventsGroupedByModel = async (
  ...
  const res = await queryPg<{ name: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByModelId = async (
  ...
  const res = await queryPg<{ modelId: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByTraceName = async (
  ...
  const res = await queryPg<{ traceName: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByPromptName = async (
  ...
  const res = await queryPg<{ promptName: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByType = async (
  ...
  const res = await queryPg<{ type: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByUserId = async (
  ...
  const res = await queryPg<{ userId: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByVersion = async (
  ...
  const res = await queryPg<{ version: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedBySessionId = async (
  ...
  const res = await queryPg<{ sessionId: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByLevel = async (
  ...
  const res = await queryPg<{ level: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByEnvironment = async (
  ...
  const res = await queryPg<{ environment: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByExperimentId = async (
  ...
  const res = await queryPg<{ experimentId: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByExperimentName = async (
  ...
  const res = await queryPg<{ experimentName: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByHasParentObservation = async (
  ...
  return queryPg<{ hasParentObservation: boolean; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByIsRootObservation = async (
  ...
  return queryPg<{ isRootObservation: boolean; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const getEventsGroupedByToolName = async (
  ...
  const res = await queryPg<{ toolName: string; count: number }>({
    query,
    params,
    tags: {
      feature: "tracing",
...
export const deleteEventsByTraceIds = async (
  ...
  await Promise.all([
    executePg(deleteOpts("events_full")),
    executePg(deleteOpts("events_core")),
  ]);
};
...
export async function getAgentGraphDataFromEventsTable(params: {
  ...
  const query = `
    SELECT
      ...
      (SELECT val FROM unnest(e.metadata_names, e.metadata_values) AS t(key, val) WHERE key = 'langgraph_node' LIMIT 1) AS node,
      (SELECT val FROM unnest(e.metadata_names, e.metadata_values) AS t(key, val) WHERE key = 'langgraph_step' LIMIT 1) AS step
    FROM events_core e
    WHERE
      e.project_id = $1
      AND e.trace_id = $2
...
> => {
  ...
  const inputSelect = truncated
    ? `left(e.input, ${env.LANGFUSE_SERVER_SIDE_IO_CHAR_LIMIT}) as input`
    : `e.input as input`;
  const outputSelect = truncated
    ? `left(e.output, ${env.LANGFUSE_SERVER_SIDE_IO_CHAR_LIMIT}) as output`
    : `e.output as output`;
  const experimentFieldsSelect = opts.includeExperimentFields
    ? `
      experiment_item_expected_output, 
      (SELECT jsonb_object_agg(key, val) FROM unnest(e.experiment_item_metadata_names, e.experiment_item_metadata_values) AS t(key, val)) as experiment_item_metadata,
    `
    : "";
  ...
  const query = `
    SELECT
      ...
      (SELECT jsonb_object_agg(key, val) FROM unnest(e.metadata_names, e.metadata_values) AS t(key, val)) as metadata
    FROM ${tableName} e
    WHERE e.project_id = $1
      AND e.span_id = ANY($2::text[])
      AND e.trace_id = ANY($3::text[])
      AND e.start_time >= $4::timestamptz
...
export const getUsersFromEventsTable = async (
  ...
  return queryPg<{ user: string; count: string }>({
    query,
    params,
    tags: {
      feature: "users",
...
export const getUsersCountFromEventsTable = async (
  ...
  const searchCondition = searchQuery
    ? `AND e.user_id ILIKE $${2 + Object.keys(appliedEventsFilter.params || {}).length}`
    : "";
  ...
  return queryPg<{ totalCount: string }>({
    query,
    params: [
      projectId,
      ...Object.values(appliedEventsFilter.params || {}),
...
export const getUserMetricsFromEventsTable = async (
  ...
  const query = `
    ...
    SELECT
      ...
      min_timestamp,
      max_timestamp,
      COALESCE((sum_usage_details->>'input')::numeric, 0) as input_usage,
      COALESCE((sum_usage_details->>'output')::numeric, 0) as output_usage,
      COALESCE((sum_usage_details->>'total')::numeric, 0) as total_usage
    FROM stats
  `;
...
export const getEventsForBlobStorageExport = function (
  ...
  queryBuilder
    .whereRaw(
      "e.start_time >= $1::timestamptz AND e.start_time <= $2::timestamptz",
...
export const getEventsForAnalyticsIntegrations = async function* (
  ...
  const queryBuilder = new EventsQueryBuilder({ projectId })
    ...
    .selectRaw(
      ...
      "(SELECT val FROM unnest(e.metadata_names, e.metadata_values) AS t(key, val) WHERE key = '$posthog_session_id' LIMIT 1) as posthog_session_id",
      "(SELECT val FROM unnest(e.metadata_names, e.metadata_values) AS t(key, val) WHERE key = '$mixpanel_session_id' LIMIT 1) as mixpanel_session_id",
    )
    .whereRaw(
      "e.start_time >= $1::timestamptz AND e.start_time < $2::timestamptz",
      [minTimestamp.toISOString(), maxTimestamp.toISOString()],
...
> => {
  ...
  const builder = new EventsAggQueryBuilder({
    projectId,
    groupByColumn:
      "(SELECT val FROM unnest(e.metadata_names, e.metadata_values) AS t(k,v) WHERE k = 'job_configuration_id' LIMIT 1)",
    selectExpression: [
      "(SELECT val FROM unnest(e.metadata_names, e.metadata_values) AS t(k,v) WHERE k = 'job_configuration_id' LIMIT 1) as evaluator_id",
      "avg(e.total_cost) as avg_cost",
      "count(*) as execution_count",
    ].join(", "),
  })
    .whereRaw("e.type = 'GENERATION'")
    .whereRaw("'job_configuration_id' = ANY(e.metadata_names)")
    .whereRaw(
      "(SELECT val FROM unnest(e.metadata_names, e.metadata_values) AS t(k,v) WHERE k = 'job_configuration_id' LIMIT 1) = ANY($1::text[])",
      [evaluatorIds],
    )
    .whereRaw("e.start_time > CURRENT_DATE - INTERVAL '7 days'");
...
export const getSessionMetricsFromEvents = async (props: {
  ...
  const builder = eventsSessionsAggregation({
    ...
    startTimeFrom: props.queryFromTimestamp
      ? props.queryFromTimestamp.toISOString()
      : undefined,
  ...
  const rows = await measureAndReturn({
    ...
    fn: async (input) =>
      queryPg<SessionEventsMetricsRow>({
        query,
        params: input.params,
        tags: input.tags,
      }),
...
export async function getLatestSdkVersionInfoFromEvents(params: {
  ...
  const builder = new EventsQueryBuilder({ projectId })
    ...
    .where({
      query: "e.scope_name != '' OR 'scope' = ANY(e.metadata_names)",
      params: {},
    })
    .orderByDefault()
    .limit(1);
...
export const getTracesIdentifierForSessionFromEvents = async (
  ...
  tracesBuilder.havingRaw("session_id = $1", [sessionId]);
