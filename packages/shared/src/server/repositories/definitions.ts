...
export const clickhouseStringDateSchema = z
  .string()
  // PG returns ISO timestamps directly, but may still contain space-formatted dates
  // from legacy data paths. Handle both formats.
  .transform((str) => str.includes(" ") ? str.replace(" ", "T") + "Z" : str)
  .pipe(z.iso.datetime());

export const pgStringDateSchema = clickhouseStringDateSchema;
