...
| Frontend | Next.js (unchanged from upstream) on `:3000`, proxying `/api/public/*` to the API |
| Ingest | `POST /api/public/otel/v1/traces` (OTLP/HTTP: protobuf, JSON, gzip) and `POST /api/public/ingestion` |

- **Design and internals**: [`docs/rust-backend-design.md`](docs/rust-backend-design.md)
- **Consuming this instance from another service (OTLP + public API)**:
  [`docs/lexqa-integration.md`](docs/lexqa-integration.md)
- **Build and run**: `docker build -t btrace .` then see §3 of the integration doc.
